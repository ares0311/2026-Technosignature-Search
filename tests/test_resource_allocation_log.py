"""Tests for resource_allocation_log operational provenance records."""
...
FIXTURE_PATH = Path(__file__).parent / "fixtures" / "resource_allocation_log.json"
...
def test_fixture_top_level_schema_version() -> None:
    obj = json.loads(FIXTURE_PATH.read_text())
    assert obj["schema_version"] == RESOURCE_ALLOCATION_LOG_SCHEMA_VERSION


def test_fixture_allocation_kinds_valid() -> None:
    obj = json.loads(FIXTURE_PATH.read_text())
    for row in obj["entries"]:
        assert row["allocation_kind"] in ALLOWED_RESOURCE_ALLOCATION_KINDS


def test_fixture_statuses_valid() -> None:
    obj = json.loads(FIXTURE_PATH.read_text())
    for row in obj["entries"]:
        assert row["status"] in ALLOWED_RESOURCE_ALLOCATION_STATUSES


def test_load_entries_returns_list() -> None:
    entries = load_resource_allocation_entries(FIXTURE_PATH)
...
def test_load_entries_are_dataclasses() -> None:
    ...
    for entry in entries:
        assert isinstance(entry, ResourceAllocationEntry)
...
def test_allocated_entries_present() -> None:
    entries = load_resource_allocation_entries(FIXTURE_PATH)
    assert any(e.status == "allocated" for e in entries)


def test_deallocated_entries_present() -> None:
    entries = load_resource_allocation_entries(FIXTURE_PATH)
    assert any(e.status == "deallocated" for e in entries)
...
def test_exhausted_entries_present() -> None:
    entries = load_resource_allocation_entries(FIXTURE_PATH)
    assert any(e.status == "exhausted" for e in entries)


def test_summary_entry_count() -> None:
    summary = resource_allocation_summary(FIXTURE_PATH)
    assert summary["entry_count"] == 5


def test_summary_allocated_count() -> None:
    summary = resource_allocation_summary(FIXTURE_PATH)
...
def test_summary_by_kind_keys() -> None:
    summary = resource_allocation_summary(FIXTURE_PATH)
    assert set(summary["by_kind"].keys()) == ALLOWED_RESOURCE_ALLOCATION_KINDS


def test_summary_by_status_keys() -> None:
    summary = resource_allocation_summary(FIXTURE_PATH)
    assert set(summary["by_status"].keys()) == ALLOWED_RESOURCE_ALLOCATION_STATUSES
...
def test_invalid_allocation_kind_raises() -> None:
    with pytest.raises(ValueError, match="allocation_kind"):
        ResourceAllocationEntry(
            entry_id="x",
            allocation_kind="invalid_kind",
            status="allocated",
            actor_id="op",
            resource_id="res",
...
def test_invalid_status_raises() -> None:
    with pytest.raises(ValueError, match="status"):
        ResourceAllocationEntry(
            entry_id="x",
            allocation_kind="compute_allocation",
