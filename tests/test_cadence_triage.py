...
from techno_search.cadence_triage import (
    CadenceHit,
    _independent_triage_outcome,
    _primary_triage_outcome,
...
def test_independent_triage_methods_agree_on_off_recurrence() -> None:
    group = [
        _hit(1_420_000_000.0, "on", "a"),
        _hit(1_420_000_000.0, "off", "b", target="comparison"),
    ]

    assert _primary_triage_outcome(group) == (
        "false_positive",
        "off_target_recurrence",
        0.99,
    )
    assert _independent_triage_outcome(
        group, ("a", "b", "c", "d", "e", "f")
    ) == (
        "false_positive"
    )
...
def test_primary_method_reserves_follow_up_for_all_three_on_scans() -> None:
    ...
    assert _primary_triage_outcome(complete)[0] == "follow_up"
    assert _primary_triage_outcome(partial)[0] == "insufficient_evidence"
...
def test_cadence_abacab_review_summary_counts_follow_up_candidates(tmp_path: Path) -> None:
    ...
    assert summary["triage_outcome_counts"] == {
        "false_positive": 1,
        "follow_up": 1,
    }
...
def test_frozen_legacy_artifact_preserves_non_claim_boundaries() -> None:
