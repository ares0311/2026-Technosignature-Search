...
def test_load_fixture_expectations() -> None:
    ...
    assert expected["expected_readiness_schema_count"] == 110
    assert expected["expected_schema_count"] == 110
...
def test_default_project_authorization_gate_passes() -> None:
    ...
    assert summary["readiness_preflight_schema_count"] == 110
    assert summary["schema_count"] == 110
