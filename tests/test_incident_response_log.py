"""Tests for incident_response_log operational provenance records."""
...
from techno_search.incident_response_log import (
    ALLOWED_INCIDENT_KINDS,
    ALLOWED_INCIDENT_STATUSES,
    INCIDENT_RESPONSE_LOG_SCHEMA_VERSION,
    IncidentResponseEntry,
    incident_response_summary,
    load_incident_response_entries,
)

FIXTURE_PATH = Path(__file__).parent / "fixtures" / "incident_response_log.json"
...
def test_fixture_top_level_schema_version() -> None:
    obj = json.loads(FIXTURE_PATH.read_text())
    assert obj["schema_version"] == INCIDENT_RESPONSE_LOG_SCHEMA_VERSION


def test_fixture_incident_kinds_valid() -> None:
    obj = json.loads(FIXTURE_PATH.read_text())
    for row in obj["entries"]:
        assert row["incident_kind"] in ALLOWED_INCIDENT_KINDS


def test_fixture_statuses_valid() -> None:
    obj = json.loads(FIXTURE_PATH.read_text())
    for row in obj["entries"]:
        assert row["status"] in ALLOWED_INCIDENT_STATUSES


def test_load_entries_returns_list() -> None:
    entries = load_incident_response_entries(FIXTURE_PATH)
...
def test_load_entries_are_dataclasses() -> None:
    ...
    for entry in entries:
        assert isinstance(entry, IncidentResponseEntry)
...
def test_open_entries_present() -> None:
    entries = load_incident_response_entries(FIXTURE_PATH)
    assert any(e.status == "open" for e in entries)


def test_contained_entries_present() -> None:
    entries = load_incident_response_entries(FIXTURE_PATH)
    assert any(e.status == "contained" for e in entries)
...
def test_closed_entries_present() -> None:
    entries = load_incident_response_entries(FIXTURE_PATH)
    assert any(e.status == "closed" for e in entries)


def test_summary_entry_count() -> None:
    summary = incident_response_summary(FIXTURE_PATH)
...
def test_summary_by_kind_keys() -> None:
    summary = incident_response_summary(FIXTURE_PATH)
    assert set(summary["by_kind"].keys()) == ALLOWED_INCIDENT_KINDS


def test_summary_by_status_keys() -> None:
    summary = incident_response_summary(FIXTURE_PATH)
    assert set(summary["by_status"].keys()) == ALLOWED_INCIDENT_STATUSES
