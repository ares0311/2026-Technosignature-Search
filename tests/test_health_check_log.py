"""Tests for health_check_log operational provenance records."""
...
from techno_search.health_check_log import (
    ALLOWED_CHECK_KINDS,
    ALLOWED_CHECK_STATUSES,
    HEALTH_CHECK_LOG_SCHEMA_VERSION,
    HealthCheckEntry,
    health_check_summary,
    load_health_check_entries,
)

FIXTURE_PATH = Path(__file__).parent / "fixtures" / "health_check_log.json"
...
def test_fixture_top_level_schema_version() -> None:
    obj = json.loads(FIXTURE_PATH.read_text())
    assert obj["schema_version"] == HEALTH_CHECK_LOG_SCHEMA_VERSION


def test_fixture_check_kinds_valid() -> None:
    obj = json.loads(FIXTURE_PATH.read_text())
    for row in obj["entries"]:
        assert row["check_kind"] in ALLOWED_CHECK_KINDS


def test_fixture_statuses_valid() -> None:
    obj = json.loads(FIXTURE_PATH.read_text())
    for row in obj["entries"]:
        assert row["status"] in ALLOWED_CHECK_STATUSES


def test_load_entries_returns_list() -> None:
    entries = load_health_check_entries(FIXTURE_PATH)
...
def test_timeout_entries_present() -> None:
    entries = load_health_check_entries(FIXTURE_PATH)
    assert any(e.status == "timeout" for e in entries)


def test_summary_entry_count() -> None:
    summary = health_check_summary(FIXTURE_PATH)
...
def test_summary_by_kind_keys() -> None:
    summary = health_check_summary(FIXTURE_PATH)
    assert set(summary["by_kind"].keys()) == ALLOWED_CHECK_KINDS


def test_summary_by_status_keys() -> None:
    summary = health_check_summary(FIXTURE_PATH)
    assert set(summary["by_status"].keys()) == ALLOWED_CHECK_STATUSES
...
def test_invalid_check_kind_raises() -> None:
    with pytest.raises(ValueError, match="check_kind"):
        HealthCheckEntry(
            entry_id="x",
            check_kind="invalid_kind",
            status="passed",
            actor_id="op",
            resource_id="res",
