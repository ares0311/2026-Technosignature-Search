...
def test_readme_has_disclaimer(tmp_path: Path) -> None:
    ...
    assert "citizen-science" in readme.lower()
...
def test_methodology_summary_has_tier1_gaps(tmp_path: Path) -> None:
    ...
    assert len(methodology["tier_1_gaps"]) == 2
...
def test_disclaimer_is_conservative() -> None:
    ...
    assert "citizen-science labels" in PEER_REVIEW_PACKAGE_DISCLAIMER


def test_generate_includes_real_label_summary(tmp_path: Path) -> None:
    generate_peer_review_package(tmp_path)
    assert (tmp_path / "real_labeled_dataset_summary.json").exists()
