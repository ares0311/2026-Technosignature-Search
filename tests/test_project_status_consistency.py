...
def test_load_project_status_expectations_fixture() -> None:
    ...
    assert expected["latest_milestone_number"] == 71
    assert expected["latest_decision_number"] == 118
    assert expected["schema_count"] == 200
...
def test_project_status_consistency_default_project_passes() -> None:
    ...
    assert summary["ok"] is True
    assert summary["expected_latest_milestone"] == 71
    assert summary["expected_latest_decision"] == 118
    assert summary["expected_schema_count"] == 200
