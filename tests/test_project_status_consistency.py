...
def test_load_project_status_expectations_fixture() -> None:
    ...
    assert expected["latest_milestone_number"] == 35
    assert expected["latest_decision_number"] == 82
    assert expected["schema_count"] == 123
...
def test_project_status_consistency_default_project_passes() -> None:
    ...
    assert summary["ok"] is True
    assert summary["expected_latest_milestone"] == 35
    assert summary["expected_latest_decision"] == 82
    assert summary["expected_schema_count"] == 123
