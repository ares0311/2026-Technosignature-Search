...
def test_load_project_status_expectations_fixture() -> None:
    ...
    assert expected["latest_milestone_number"] == 37
    assert expected["latest_decision_number"] == 84
    assert expected["schema_count"] == 125
...
def test_project_status_consistency_default_project_passes() -> None:
    ...
    assert summary["ok"] is True
    assert summary["expected_latest_milestone"] == 37
    assert summary["expected_latest_decision"] == 84
    assert summary["expected_schema_count"] == 125
