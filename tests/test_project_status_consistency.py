...
def test_load_project_status_expectations_fixture() -> None:
    ...
    assert expected["latest_milestone_number"] == 33
    assert expected["latest_decision_number"] == 80
    assert expected["schema_count"] == 121
...
def test_project_status_consistency_default_project_passes() -> None:
    ...
    assert summary["ok"] is True
    assert summary["expected_latest_milestone"] == 33
    assert summary["expected_latest_decision"] == 80
    assert summary["expected_schema_count"] == 121
