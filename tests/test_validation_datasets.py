...
def test_validation_dataset_manifest_records_phase0_synthetic_removal() -> None:
...
def test_validation_dataset_summary_counts_manifest_coverage() -> None:
    ...
    assert summary["total_case_count"] == 0
    assert summary["false_positive_class_count"] == 0
    assert summary["expected_pathway_count"] == 0
    assert summary["by_track"] == {}
    assert summary["by_dataset_kind"] == {}
    assert summary["by_readiness"] == {}
