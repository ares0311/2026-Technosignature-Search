#!/usr/bin/env bash
# download_bl_hits.sh — obtain sample turboSETI hit tables for pipeline testing
#
# This script generates synthetic turboSETI .dat files in the exact real format
# so the pipeline can be tested end-to-end locally.
#
# For REAL BL data, visit:
#   https://seti.berkeley.edu/listen/data.html
#   https://breakthroughinitiatives.org/opendatasearch
# Download .dat files from a published BL data release and place them in
#   ~/technosignature-data/bl_hits/
# then run scripts/run_pipeline_on_bl_data.sh.
#
...
echo ""
echo "NOTE: These are synthetic files in real turboSETI format."
echo "      For real BL data see: https://seti.berkeley.edu/listen/data.html"
echo ""
...
for source, mjd, ra, dec in TARGETS:
    ...
    with open(filepath, "w") as f:
        ...
        for i in range(1, n_hits + 1):
            ...
            f.write(f"{row}\n")
    print(f"  Created: {filename} ({n_hits} hits)")
