...
Operational background logs must be stored in a top-level SQLite database under `logs/`. JSON ledgers remain small fixtures and compatibility artifacts, but SQLite is the local operational source of truth for scheduled runs. The SQLite log must preserve run provenance, config version, code commit, target-selection rationale, negative evidence, blocking issues, and exactly one reviewed or needs-follow-up outcome per run. It must keep network access disabled by default and must not record external submission approval unless the user has explicitly recorded that decision.
...
Report manifests include a `provenance_summary` object with schema/config versions, source IDs, source dataset, provider name and service URL when applicable, query parameters when applicable, cache key when applicable, generated timestamp, and code commit when available.
