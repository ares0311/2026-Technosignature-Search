...
| Detection-optimized target selection algorithm | ⚠️ 3a (novel-target selection) metadata discovery and size preflight are complete for the full current HPRC queue: `top25`/`next25`/`batch3`-`batch13` plus the 1,358-target `batch14_bulk` round. The queue has 949 sized URL-available targets (~239.06 GB), 540 completed no-product results, and 214 already-acquired controls (2026-07-17 coverage-state fix: 198 Step 0 `stream_process_evict` completions were previously misclassified as still needing download approval). This is an inventory, not raw-download approval, and cannot fit the permanent 100 GB cap as a bulk pull. 3b remains design-only per its documented gate (no real qualifying candidate exists yet). See Step 3a. |
