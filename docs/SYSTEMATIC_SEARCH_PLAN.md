...
| Detection-optimized target selection algorithm | ⚠️ 3a (novel-target selection) metadata discovery and size preflight are complete for the full current HPRC queue: `top25`/`next25`/`batch3`-`batch13` plus the 1,358-target `batch14_bulk` round. The queue has 751 sized URL-available targets (~189.10 GB), 540 completed no-product results, and 412 already-acquired controls (2026-07-17: a coverage-state fix recognized 198 Step 0 `stream_process_evict` completions previously misclassified as still needing download approval, and the approved `step3a_batch1` batch, 198 more targets, was then downloaded/processed/evicted the same day). This is an inventory, not raw-download approval, and cannot fit the permanent 100 GB cap as a bulk pull. 3b remains design-only per its documented gate (no real qualifying candidate exists yet). See Step 3a. |
