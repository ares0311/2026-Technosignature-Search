...
| Task | Status |
|---|---|
| JWST NIRSpec/NIRISS transmission spectra ingest (MAST) | ⚠️ Corrected + partial, 2026-07-03 — real research (live web search, not memory) found the actual real-world instrument for the CFC/PFC/SF₆/NF₃ group this project's first gas set targets is **JWST MIRI Low Resolution Spectrometer (5-14 μm, R~40-160)**, not NIRSpec/NIRISS as this roadmap line originally assumed (Schwieterman et al. 2024, ApJ 969, 20, used MIRI LRS specifically for this gas group). `spectroscopy/jwst_spectrum_io.py` ingests the real JWST pipeline `x1d` product format (`EXTRACT1D` FITS table extension with `WAVELENGTH`/`FLUX`/`FLUX_ERROR` columns, verified against the official JWST pipeline docs). Live MAST search/download is not yet built: the real `instrument_name` query-field value MAST uses for MIRI LRS observations could not be confirmed via a citable source this session, and rather than guess a filter that could silently return zero results, this is deferred pending the user's real MAST access (same live-network restriction as Track A catalogs, Phase 2 photometry, and Phase 3 WISE). |
...
| CFC/HFC (no natural source) detection | ✅ Done — `spectroscopy/technosignature_gases.py` checks for real statistically significant absorption at the real, independently-verified band centers of CF4 (7.80 μm) and C2F6 (8.00 μm and 8.95 μm) — cross-checked via live web search against independent spectroscopy literature, 2026-07-03. C3F8 (the fifth gas in Schwieterman et al. 2024) is explicitly NOT included: its real band center could not be verified via a citable source and was not guessed. |
| N₂O (agricultural enhancement) detection | ❌ Not started, with a real reason found rather than assumed — N₂O has two real fundamental bands, both cross-verified via live web search: ν3 at 4.5 μm and ν1 at 7.8 μm. The 4.5 μm band falls below MIRI LRS's 5 μm floor (not observable with this instrument). The 7.8 μm band is observable, but coincides almost exactly with CF4's real 7.80 μm band already implemented here — a real spectral degeneracy that would misattribute a detected feature to the wrong gas without additional diagnostic information this project doesn't have (e.g. resolved line shape at higher spectral resolution, or a second N2O band to cross-check against). N₂O also requires "enhancement over natural background" interpretation (it has a real natural biogenic source, unlike the purely artificial CF4/C2F6/SF6/NF3 group), which a bare band-position check cannot provide. Not implemented rather than implemented ambiguously. |
| SF₆ (electrical insulation, no natural source) detection | ✅ Done — real, verified band center 10.55 μm (cross-checked via live web search). NF3 (11.02 μm, 907.54 cm⁻¹) is also implemented as a related artificial gas with no natural source, from the same Schwieterman et al. 2024 gas set. |
| Comparison to photochemical equilibrium models | ❌ Not started — this remains a full radiative-transfer/photochemistry undertaking requiring real molecular absorption cross-section grids (e.g. HITRAN), not available from this sandbox; the current implementation is an honestly-scoped first-pass band-position significance check, not a full retrieval (documented in `technosignature_gases.py`, the same kind of scope limitation as the WISE blackbody-vs-Kurucz-grid distinction). |
| Spectral anomaly candidate output with significance | ✅ Done — `spectroscopy/prototype.py`'s `build_spectroscopy_candidate()` produces a `Track.SPECTROSCOPY` `Candidate` with real per-band significance features (`<gas>_<band>um_significance_sigma`), a real 5-sigma detection-count aggregate (the standard physics/astronomy discovery-significance convention, not invented), and `technosignature_gas_score`; wired into `pipeline_runner.run_pipeline(..., track="spectroscopy")` and `scoring.py`'s `_spectroscopy_scores()`. Verified end-to-end on a real, locally-generated x1d FITS fixture with an injected SF6-band dip: recovered significance 61.7σ, correctly flat (<4σ) on the other three verified bands. |

**References:** Lin et al. 2014 (ApJ 792, L7), Schwieterman et al. 2018 (Astrobiology 18, 663), Schwieterman et al. 2024 (ApJ 969, 20, "Artificial Greenhouse Gases as Exoplanet Technosignatures" — the real source for the CF4/C2F6/SF6/NF3 band centers used here)
