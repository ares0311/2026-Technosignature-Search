...
| Task | Status |
|---|---|
| JWST NIRSpec/NIRISS transmission spectra ingest (MAST) | ⚠️ Corrected + partial, 2026-07-03 — real research (live web search, not memory) found the actual real-world instrument for the CFC/PFC/SF₆/NF₃ group this project's first gas set targets is **JWST MIRI Low Resolution Spectrometer (5-14 μm, R~40-160)**, not NIRSpec/NIRISS as this roadmap line originally assumed (Schwieterman et al. 2024, ApJ 969, 20, used MIRI LRS specifically for this gas group). `spectroscopy/jwst_spectrum_io.py` ingests the real JWST pipeline `x1d` product format (`EXTRACT1D` FITS table extension with `WAVELENGTH`/`FLUX`/`FLUX_ERROR` columns, verified against the official JWST pipeline docs). **Update, 2026-07-03 — the MAST blocker is now resolved**: the user's research agent ran a real, live `astroquery.mast.Observations.query_criteria()` call and confirmed the real `instrument_name` values for MIRI LRS observations are `MIRI/SLIT` (1526 real observations) and `MIRI/SLITLESS` (742 real observations), additionally filterable by `filters="P750L"` (the LRS prism disperser); `MIRI/LRS`, `MIRI/SLITLESSPRISM`, and `MIRI/LRS-FIXEDSLIT` were live-tested and definitively return zero results. Full findings recorded in `docs/technosignature_detection_research_answers.md`. A live MAST search/download CLI wrapper (mirroring `photometry/lightcurve_search.py`) is now unblocked and is the next concrete Phase 4 step. |
...
| Comparison to photochemical equilibrium models | ❌ Not started — this remains a full radiative-transfer/photochemistry undertaking that would convolve the full downloaded HITRAN cross-section grids (now available locally, ignored under `hitran_xsc_downloads/`) through an atmospheric model, not just check a peak wavelength; the current implementation is an honestly-scoped first-pass band-position significance check, not a full retrieval (documented in `technosignature_gases.py`, the same kind of scope limitation as the WISE blackbody-vs-Kurucz-grid distinction). |
...
excluded for lack of a citable band center). Live MAST search/download
(`jwst_search.py`) is now unblocked — the real MAST `instrument_name` field
values for MIRI LRS (`MIRI/SLIT`, `MIRI/SLITLESS`, filterable by
`filters="P750L"`) were confirmed via a real live `astroquery` query — but
the wrapper module itself has not yet been built.
