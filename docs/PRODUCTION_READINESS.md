...
| Task | Status |
|---|---|
| Track A known-explanation classifier before Track B `unknown_candidate` routing | ⚠️ Partial — **real, executed baseline as of 2026-07-02** (Phases 0-2 of `docs/technosignature_datasets_agent_brief.md` complete). HTRU2 (17,898 rows, verified exact match) trained 3 baseline models: `logistic_regression` (F1=0.883, precision=0.948, recall=0.826), `hist_gradient_boosting` (F1=0.895, precision=0.920, recall=0.872), `random_forest` (F1=0.896, precision=0.925, recall=0.869 — **selected as best model**, saved to `models/track_a/htru2_random_forest.joblib`, local/ignored). All four Phase 2 catalogs acquired from live hosts: ATNF pulsars (4,393 rows), CHIME/FRB Catalog 1 (600 rows, exceeds the brief's 536-burst snapshot), Roma-BZCAT blazars/AGN (3,561 rows, exact match), Fermi 4FGL-DR4 gamma-ray sources (7,195 rows, one more than the brief's 7,194-row snapshot — real catalog drift, not a bug). Two real acquisition bugs found and fixed live: Roma-BZCAT's VizieR `RAJ2000`/`DEJ2000` columns are sexagesimal text (not decimal degrees like CHIME/FRB's same-named columns) and Fermi 4FGL-DR4's FITS table has multidimensional per-band flux columns that crash direct `to_pandas()`. **CelesTrak/SatNOGS satellite-transmitter matching implemented as of 2026-07-02** — `src/techno_search/track_a_satellites.py`: `track-a-satellite-acquire {celestrak_satcat, celestrak_gp_active, satnogs_satellites, satnogs_transmitters, satnogs_tle, satnogs_modes}` acquires the 6 raw sources; `track-a-satellite-match <frequency_hz> <observation_time_utc> <ra_deg> <dec_deg> --observer-lat-deg --observer-lon-deg --observer-elevation-m` implements the brief's matching rule (frequency overlaps a known SatNOGS transmitter range AND SGP4-propagated position at the observation time falls within the beam radius) via `skyfield.EarthSatellite.from_omm()` against CelesTrak's real CCSDS OMM JSON schema (verified against the installed library source, not assumed). Observer geographic location is a required caller-supplied parameter — no telescope coordinate is hardcoded anywhere in this module. All four Phase 2 satellite/transmitter catalogs are implemented but not yet acquired against live hosts (requires network this sandbox cannot reach). **Phase 3 (Small Historical Replay) complete and executed against real data on 2026-07-02** — `techno-search track-a-historical-replay --sample-size 3` (`src/techno_search/track_a_replay.py`) ran against the user's real locally-acquired catalogs: **13/13 replay cases recovered (`all_recovered: true`)** — 3 real ATNF pulsars (J0002+6216, J0006+1834, J0007+7303), 3 real CHIME/FRB bursts (FRB20180725A, FRB20180727A, FRB20180729A), 3 real Roma-BZCAT blazars (5BZQ J0000-3221, 5BZQ J0001-1551, 5BZQ J0001+1914), 3 real Fermi 4FGL gamma-ray sources (4FGL J0000.3-7355, 4FGL J0000.5+0743, 4FGL J0000.7+2530), and the fixed empty-sky negative control correctly reported `no_known_match` with all 4 catalogs loaded. **This satisfies the brief's explicit gate ("Do not start Track B until Track A has a tested, reproducible baseline" + "Track B starts only after Track A and historical replay work") — Track A + historical replay are both now genuinely done with real data.** **Track B Phase 4 gate implemented as of 2026-07-02** — `src/techno_search/track_b_gate.py`: `track_b_unknown_candidate_gate()` combines all nine brief conditions (catalog cross-match for pulsar/FRB/blazar-AGN/gamma, satellite-transmitter match, RFI band/database overlap, instrumental artifact score, ABACAB cadence score, semisupervised anomaly score, source provenance) as a separate, additive function per the researched design (CHIME/FRB's staged L1b→L2/L3→L4 architecture; not wired into the shared `Pathway` enum/`score_candidate()`). Condition 8 ("high anomaly/OOD score") has no calibrated threshold anywhere in this repo, so it is reported as raw evidence with `satisfied=None` rather than an invented cutoff — an unresolved condition blocks `eligible_for_unknown_candidate` by construction, so the gate can never emit an unsupported claim. Real live data as of 2026-07-02: `celestrak_gp_active` (15,932 rows) and `satnogs_transmitters` (4,904 rows) acquired successfully — the two catalogs the satellite matcher actually reads — plus `satnogs_satellites` (2,716 rows). `celestrak_satcat` initially failed against the live host (`Invalid query: "FORMAT=CSV"` — the brief's documented `records.php?FORMAT=CSV` query is rejected by the current API, which requires an actual query key). Root cause confirmed live: `records.php` needs `CATNR`/`INTDES`/`GROUP`/`NAME`/`SPECIAL`, and `GROUP=all` (a plausible-looking guess) does not exist either — confirmed by testing both against the real host rather than assuming. Fixed to the verified working static full-catalog dump `https://celestrak.org/pub/satcat.csv` (confirmed real header row + real data via live `curl`). This never blocked satellite-transmitter matching, which only reads `gp_active`/`transmitters`, not SATCAT. Not yet run against real radio candidates — CLI wiring for `track_b_unknown_candidate_gate()` and end-to-end real-candidate validation remain open. |
