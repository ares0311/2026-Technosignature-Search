...
```bash
caffeinate -i .venv/bin/techno-search scan-summary data/bl_hits/ \
  --output-dir results/scan_$(date +%Y%m%d)/
```

This runs the pipeline on each target directory, applies cross-target RFI
suppression, and produces a ranked anomaly report in `results/scan_<date>/`.

Progress is printed per target:
```
[1/10] Scoring HIP99427 ... done (12 candidates, 0 follow_up)
[2/10] Scoring HIP100670 ... done (8 candidates, 0 follow_up)
...
```
...
```bash
cat results/scan_$(date +%Y%m%d)/scan_summary.json | \
  .venv/bin/python -m json.tool | head -100
```

The summary lists candidates ranked by score. Columns:
- `rank` — score rank across all targets (1 = highest)
- `candidate_id` — unique identifier
- `target_name` — which star
- `frequency_hz` — signal frequency
- `snr` — signal-to-noise ratio
- `recommended_pathway` — `do_not_submit_false_positive` / `human_review_queue` / `candidate_review_packet`
- `cross_target_rfi_flagged` — True if this frequency appeared in ≥2 targets (almost certainly RFI)

Focus on candidates where:
- `recommended_pathway == "candidate_review_packet"` AND
- `cross_target_rfi_flagged == false` AND
- `snr >= 42.4` (calibrated noise floor)

---

## Step 5 — Check the review dashboard
...
## Step 6 — Escalation gate (if a compelling candidate is found)
...
## Step 7 — Archive the scan results

```bash
git add results/scan_$(date +%Y%m%d)/scan_summary.json
git add results/scan_$(date +%Y%m%d)/*.json
...
## Step 8 — Long-term: automated weekly scan
