...
Output fields: `disclaimer`, `schema_version`, `ok`, `issue_count`, `issues`, Tier 1 blocker counts, missing blocker phrases, admission blocker counts, real-data authorization totals, external-submission authorization totals, and network-access counts. This is a local readiness visibility gate only; it does not ingest real observation data, calibrate thresholds, clear blockers, authorize live data, or approve external submission.
